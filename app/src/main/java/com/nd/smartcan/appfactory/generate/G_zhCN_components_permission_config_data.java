...
public class G_zhCN_components_permission_config_data implements IJsonMapCreator
{
	...
	public Map<String, Object> _createMap()
	{
		...
		stack.push(map1);
		{
			...
			map1.put("MICROPHONE_intro", "允许后，您就可以进行语音聊天、录制语音、小视频的相关操作。");
			map1.put("CAMERA_intro", "允许后，您就可以在101 Growth进行扫描二维码、拍摄照片或视频（用于修改头像/信息统计等）的相关操作");
			map1.put("ALERT_WINDOW_intro", "允许后，您就可以在开启护眼模式时显示护眼蒙版和使用时间提醒弹窗的功能。");
			map1.put("PHONE_intro", "为您提供信息推送和安全风控服务");
			map1.put("LOCATION_intro", "允许后，您就可以进行地理定位、好友实时位置共享的关于定位的相关操作。");
